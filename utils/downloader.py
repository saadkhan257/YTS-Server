...
def extract_metadata(url, headers=None, download_id=None):
    ...
    try:
        ...
        for f in formats:
            ...
            if lang and f.get("vcodec") == "none" and f.get("acodec") != "none":
                if lang.lower() not in seen_dubs:
                    seen_dubs.add(lang.lower())
                    dubs.append({
                        "lang": lang.lower(),
                        "label": lang.upper()
                    })
        ...
        for f in formats:
            if f.get("vcodec") != "none" and f.get("height") and f.get("ext") in MP4_EXTENSIONS:
                label = f"{f['height']}p"
                if label in seen:
                    continue
                seen.add(label)
